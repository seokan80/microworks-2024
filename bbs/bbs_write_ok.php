...
if( $_POST[name] ) {
	...
	// 답변
	if( $ref ) {
		$db->update("cs_bbs_data", "re_step=re_step+1 where ref='$ref' and re_step > '$re_step'");
		$re_step++;
		$re_level++;
	} else {		// 쓰기
	...
	################# plupload 이미지 처리 #################
		...
		$tools->alertJavaGo($title_complete, $returnURL); 
		} else {
			@unlink($ROOT_DIR."/data/bbsData/".$file_name); 
			@unlink($ROOT_DIR."/data/bbsData/".$file_name2); 
			@unlink($ROOT_DIR."/data/bbsData/".$file_name3); 
			@unlink($ROOT_DIR."/data/bbsData/".$file_name4); 
			@unlink($ROOT_DIR."/data/bbsData/".$file_name5); 
