...
$bbs_view_del	=	$tools->filter($_POST[bbs_view_del]);

if( $bbs_view_del ) {

	$pwd = $tools->openssl($tools->filter($_POST[pwd]));

	$bbs_stat = $db->object("cs_bbs_data", "where idx = '$idx'", "pwd, bbs_file, bbs_file2, bbs_file3, bbs_file4, bbs_file5");
	
	if($_SESSION['USERID']){

			// 자료실 삭제
			if(empty($bbs_stat->sbbs_file))		{ @unlink("../data/bbsData/".$bbs_stat->bbs_file); }
			if(empty($bbs_stat->sbbs_file2))	{ @unlink("../data/bbsData/".$bbs_stat->bbs_file2); }
			if(empty($bbs_stat->sbbs_file3))	{ @unlink("../data/bbsData/".$bbs_stat->bbs_file3); }
			if(empty($bbs_stat->sbbs_file4))	{ @unlink("../data/bbsData/".$bbs_stat->bbs_file4); }
			if(empty($bbs_stat->sbbs_file5))	{ @unlink("../data/bbsData/".$bbs_stat->bbs_file5); }
			// 코멘트 삭제
