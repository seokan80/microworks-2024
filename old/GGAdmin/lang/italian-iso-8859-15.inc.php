...
$strAllowInterrupt = 'Permette di interrompere il processo di importazione nel caso lo script rilevi che ?troppo vicino al tempo limite. Questo potrebbe essere un buon modo di importare grandi file, tuttavia potrebbe interrompere la transazione.';
...
$strBrowse = 'Mostra';
$strBufferPoolActivity = 'Attivit?del Buffer Pool';
...
$strCompressionWillBeDetected = 'Il tipo di compressione del file importato sar?automaticamente rilevato da: %s';
...
$strCopy = 'Copia';
$strCopyDatabaseOK = 'Il Database %s ?stato copiato in %s';
...
$strCouldNotKill = 'phpMyAdmin non ?in grado di terminare il thread %s. Probabilmente ?gi?stato terminato.';
$strCreate = 'Crea';
...
$strDatabaseEmpty = 'Il nome del DataBase ?vuoto!';
...
$strDefaultEngine = '%s ?il motore di memorizzazione predefinito su questo server MySQL.';
...
$strEnd = 'Fine';
$strEndRaw = 'FINE RAW';
$strEngineAvailable = '%s ?disponibile su questo server MySQL.';
$strEngineDisabled = '%s ?stato disabilitato su questo server MySQL.';
$strEngines = 'Motori';
...
$strFileCouldNotBeRead = 'Il file non pu?essere letto';
$strFileNameTemplateDescriptionDatabase = 'nome database';
$strFileNameTemplateDescription = 'Questo valore ?interpretato usando %1$sstrftime%2$s, in questo modo puoi usare stringhe di formattazione per le date/tempi. Verranno anche aggiunte le seguenti trasformazioni: %3$s. Il testo rimanente rester?invariato.';
...
$strIndexWarningMultiple = 'Pi?di una chiave %s ?stata creata per la colonna `%s`';
...
$strInternalNotNecessary = '* Non ?necessaria una relazione interna quando gi?esiste in InnoDB.';
...
$strInvalidRowNumber = '%d non ?un numero valido di righe.';
...
$strMbExtensionMissing = 'L\'estensione PHP mbstring non ?stata trovata e sembra che si stia utilizzando un set di caratteri multibyte. Senza l\'estensione mbstring, phpMyAdmin non ?in grado di dividere correttamente le stringhe di caratteri e questo pu?portare a risultati inaspettati.';
$strMbOverloadWarning = 'Avete abilitato mbstring.func_overload nella configurazione del PHP. Questa opzione ?incompatibile con phpMyAdmin e potrebbe causare la corruzione di alcuni dati!';
...
$strMyISAMDataPointerSizeDesc = 'Dimensione del puntatore predefinito in Bytes, che deve essere usata da CREATE TABLE per le tabelle MyISAM quando non ?stata specificata l\'opzione MAX_ROWS.';
$strMyISAMDataPointerSize = 'Domensione del puntatore dati';
$strMyISAMMaxExtraSortFileSizeDesc = 'Se il file temporaneo ?usato per la creazione veloce di un indice MyISAM, occuperebbe pi?spazio dell\'utilizzo del metodo key cache con la quantit?ivi specificata: perci?si deve prediligere il metodo key cache.';
...
$strMyISAMRecoverOptionsDesc = 'La modalit?di irppristino automatico di tabelle MyISAM corrotte, come impostato tramite l\'opzione di lan cio del server --myisam-recover.';
$strMyISAMRecoverOptions = 'Modalit?di ripristino automatico';
$strMyISAMRepairThreadsDesc = 'Se questo valore ?maggiore di 1, gli indici della tabella MyISAM vengono creati in parallelo (ogni indice nel suo thread) durante il processo di ordinamento Repair by.';
...
$strName = 'Nome';
$strNext = 'Prossimo';
$strNoActivity = 'Nessuna attivit?da %s secondi o pi? si prega di autenticarsi nuovamente';
$strNoDatabases = 'Nessun database';
$strNoDatabasesSelected = 'Nessun database selezionato.';
$strNoDescription = 'nessuna Description';
$strNoDetailsForEngine = 'Non ?disponibile nessuna informazione dettagliata sullo stato di questo motore di memorizzazione.';
...
$strRenameDatabaseOK = 'Il DataBase %s ?stato rinominato in %s';
...
$strShowStatusCreated_tmp_disk_tablesDescr = 'Il numero delle tabelle temporanee create automaticamente sul disco dal server mentre esegue i comandi. Se il valore Created_tmp_disk_tables ?grande, potresti voler aumentare il valore tmp_table_size, per fare im modo che le tabelle temporanee siano memory-based anzich?disk-based.';
...
$strShowStatusDelayed_insert_threadsDescr = 'Il numero di processi INSERT DELAYED in uso. Ciascuna tabella su cui ?usato INSERT DELAYED occupa un thread.';
...
$strShowStatusHandler_deleteDescr = 'Il numero di volte in cui una riga ?stata cancellata da una tabella.';
$strShowStatusHandler_discoverDescr = 'Il server MySQL pu?chiedere al motore di storage NDB Cluster se conosce una tabella sulla base di un nome dato. Questo ?chaiamto discovery. Handler_discover indica il numero di volte che una tabella ?stata trovata.';
$strShowStatusHandler_read_firstDescr = 'Il numero di volte che il primo valore ?stato letto da un indice. Se ?troppo alto ?probabile che il server stia facendo molte scansioni complete degli indici; per esempio, SELECT col1 FROM foo, assumento che col1 sia indicizzata.';
$strShowStatusHandler_read_keyDescr = 'Il numero di richieste per leggere una riga basata su di una chiave. Se ?alta, ?un buon indice che le tue query e le tue tabelle sono correttamente indicizzate.';
$strShowStatusHandler_read_nextDescr = 'Il numero di richieste per leggere la riga successiva nell\'ordine delle chiavi. Questo valore ?incrementato se stai facendo una query su di una colonna indice con un range costante, oppure se stai facendo una scansione degli indici.';
$strShowStatusHandler_read_prevDescr = 'Il numero di richieste per leggere la riga precedente nell\'ordine delle chiavi. Questo metodo di lettura ?principalmente utilizzato per ottimizzare ORDER BY ... DESC.';
$strShowStatusHandler_read_rndDescr = 'Il numero di richieste per leggere una riga basata su una posizione fissa. Questo valore ?alto se stai facendo molte richieste che richiedono un ordinamento dei risultati. Probabilmente hai molte query che che richiedono a MySQL di leggere l\'intera tabella oppure ci sono dei joins che non usano le chiavi correttamente.';
...
$strShowStatusInnodb_buffer_pool_pages_miscDescr = 'Il numero di pagine occupate perch?sono state allocate per amministrazione, come row locks o per hash index adattivi. Questo valore pu?essere calcolato come Innodb_buffer_pool_pages_total - Innodb_buffer_pool_pages_free - Innodb_buffer_pool_pages_data.';
...
$strShowStatusInnodb_data_readDescr = 'La quantit?di dati letti fino ad ora, in bytes.';
...
$strShowStatusInnodb_data_writtenDescr = 'La quantit?di dati scritti fino ad ora, in bytes.';
...
$strShowStatusInnodb_log_waitsDescr = 'Il numero di attese che abbiamo avuto perch?il buffer di log era troppo piccolo e abbiamo duvuto attendere che fosse aggiornato prima di continuare.';
...
$strShowStatusKey_blocks_unusedDescr = 'Il numero di blocchi non usati nella cache chiave. Puoi usare questo valore per determinare quanta cache chiave ?in uso.';
$strShowStatusKey_blocks_usedDescr = 'Il numero di blocchi usati nella cache chiave. The number of used blocks in the key cache. Questo valore ?un\'importante segnale che indica il numero massimo di blocchi che sono stati in uso contemporaneamente.';
$strShowStatusKey_read_requestsDescr = 'Il numero di richieste per le ggere un blocco chiave dalla cache.';
$strShowStatusKey_readsDescr = 'Il numero di letture fisiche dal disco di un blocco chiave. Se Key_reads ?grande allora il valore key_buffer_size ?probabilmente troppo piccolo. IIl rapporto di cache miss rate pu?essere calcolato come Key_reads/Key_read_requests.';
...
$strShowStatusLast_query_costDescr = 'Il costo totale dell\'ultima query compilata cos?come computato dall\'ottimizzatore delle query. Utile per comparare il costo di differenti query per la stessa operazione di query. Il valore di default ?0, che significa che nessuna query ?stata ancora compilata.';
$strShowStatusNot_flushed_delayed_rowsDescr = 'In numero di righe in attesa di essere scritte nella coda INSERT DELAY.';
$strShowStatusOpened_tablesDescr = 'Il numero di tabelle che sono state aperte. Se il valore opened_tables ?grande, probabilmente il valore di table cache ?troppo piccolo.';
...
$strShowStatusQcache_not_cachedDescr = 'Il numero di query non in cache (impossibilit?di inserirle nella cache oppure non inserite per i settaggi del parametro query_cache_type).';
...
$strShowStatusSelect_full_joinDescr = 'Il numero di joins che non usano gli indici. (Se questo valore non ?0, dovresti controllare attentamente gli indici delle tue tabelle.)';
...
$strShowStatusSelect_range_checkDescr = 'Il numero di joins senza chiavi che controllano per l\'uso di una chiave dopo ogni riga. (Se questo valore non ?0, dovresti controllare attentamente gli indici delle tue tabelle.)';
$strShowStatusSelect_rangeDescr = 'Il numero di joins che usano un range sulla prima tabella. (Non ? solitamente, un valore critico anche se ?grande.)';
...
$strShowStatusSlave_runningDescr = 'Questa chiave ?ON se questo ?un server slave connesso ad un server master.';
...
$strShowStatusSort_merge_passesDescr = 'Il numero di fusioni passate all\'algoritmo di ordianemento che sono state fatte. Se questo valore ?grande, dovresti incrementare la variabile di sistema sort_buffer_size.';
...
$strShowStatusTable_locks_immediateDescr = 'Il numero di volte che un table lock ?stato eseguito immediatamente.';
$strShowStatusTable_locks_waitedDescr = 'Il numero di volte che un table lock ?stato eseguito immediatamente ed era necessaria un\'attesa. Se ?alto, potresti avere dei problemi con le performance, dovresti prima ottimizzare le query, oppure sia utilizzare le repliche, sia dividere le tabelle.';
$strShowStatusThreads_cachedDescr = 'Il numero dei processi nella cache dei processi. L\'hit rate della cache pu?essere calcolato come processi_creati/connessioni. Se questo valore ?rosso devi aumentare la tua thread_cache_size.';
$strShowStatusThreads_connectedDescr = 'Il numero di connessioni correntemente aperte.';
$strShowStatusThreads_createdDescr = 'Il numero di processi creati per gestire le connessioni. Se Threads_created ?grosso, devi probabilmente aumentare il valore thread_cache_size. (Normalmente questo non fornisce un significatico incremento delle performace se hai una buona implementazione dei processi.)';
...
$strSocketProblem = '(o il socket del server locale MySQL non ?correttamente configurato)';
...
$strSQLExportCompatibility = 'Compatibilit?dell\'esportazione SQL';
...
$strTableAlreadyExists = 'La tabella %s esiste gi?';
...
$strTimeoutNothingParsed = 'Nell\'ultima esecuzione nessun dato ?stato processato, questo, solitamente, vuole dire che che phpMyAdmin non ?in grado di ultimare l\'operazione fino a che non verr?aumentato il parametro php time limits.';
$strTimeoutPassed = 'Superato il tempo limite dello script, se vuoi finire l\'importazione inoltra nuovamente il file e il processo riprender?';
$strTime = 'Tempo';
...
$strUnsupportedCompressionDetected = 'Stai cercando di importare un file con un tipo di compressione non supportato. Altrimenti il supporto per questo tipo di compressione non ?stato ancora implementato o ?stato disabilitato dalla tua configurazione.';
...
$strViewHasBeenDropped = 'La vista %s ?stata eliminata';
